    ...
    def load_metadata(self, img_dir, gt_dir):
        ...
        for img_fp in img_fps:
            ...
            if not(os.path.exists(gt_fp)):
                gt_file_name = "{}.txt".format(img_fp.split("/")[-1])
                gt_fp = os.path.join(gt_dir, gt_file_name)

            assert os.path.exists(img_fp), "This image path is not exist"
            assert os.path.exists(gt_fp), "This gt path is not exist"
            gt_fps.append(gt_fp)
...
class MixDatasetIter(BaseDatasetIter):
    
    def __init__(self, train_dir, train_gt_dir, ignore_tags, **kwargs):
        super().__init__(train_dir, train_gt_dir, ignore_tags, **kwargs)
    
    def load_metadata(self, image_dir, gt_dir=None):
        image_paths = []
        gt_paths = []
        
        for file_name in os.listdir(image_dir):
            if file_name == "X51006913019.jpg":
                continue
            
            if ".jpg" in file_name or ".png" in file_name:
                image_path = os.path.join(image_dir, file_name)
                gt_name = "{}.txt".format(file_name.split(".")[0])
                gt_path = os.path.join(gt_dir, gt_name)
                
                if not(os.path.exists(gt_path)):
                    print(gt_path, image_path)
                    continue
                
                image_paths.append(image_path)
                gt_paths.append(gt_path)
        
        assert len(image_paths) == len(gt_paths), "The number of image conflicts with the number of ground truth"
    ...
    def load_all_anns(self, gt_paths):
        ...
        for gt_fp in gt_paths:
            lines = []
            with open(gt_fp, 'r', encoding="cp1252") as f:
                for line in f:
                    item = {}
                    gt = line.strip().strip('\ufeff').strip(
                        '\xef\xbb\xbf').split(",")
