...
def get_data_loaders(cfg):
    ...
    if dataset_name == 'icdar2015':
        ...
        from data_loaders import MixDatasetIter
        TextDatasetIter = MixDatasetIter
    elif dataset_name == 'ctw':
        from data_loaders import CTW1500
        TextDatasetIter = CTW1500
    else:
...
def main(cfg):
    ...
    for epoch in range(cfg.hps.no_epochs):
        print("Epoch {}/{}".format(epoch + 1, cfg.hps.no_epochs))
        ...
        if hmean >= best_hmean:
            ...
            torch.save(
                dbnet.state_dict(),
                os.path.join(cfg.meta.root_dir, "{}_f1:{}_pre:{}_recall:{}.pth".format(cfg.model.best_hmean_cp_path.split(".")[0], hmean, precision, recall)))
...
@hydra.main(config_path="../example_config.yaml")
def run(cfg):
    main(cfg)
