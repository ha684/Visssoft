...
from source.dbnet.postprocess import SegDetectorRepresenter
...
def draw_bbox(img, result, color=(128, 0, 128), thickness=1):
    """
    :input: RGB img
    """
...
def visualize_heatmap(args, img_fn, tmp_img, tmp_pred):
    ...
    img_fn = "heatmap_result_{}.png".format(img_fn)
    plt.savefig(os.path.join(args.save_dir, img_fn),
                dpi=200,
                bbox_inches='tight')
    gc.collect()
    
def return_bbox(batch, preds, thresh=0.5, box_thresh=0.7, unclip_ratio=1.5, is_output_polygon=False):
